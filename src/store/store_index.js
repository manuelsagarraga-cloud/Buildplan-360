...
export const useStore = create((set, get) => ({
  ...
  init: async () => {
    try {
      ...
      // Frescura: solo la última tarea actualizada por proyecto (evita traer todas las tareas)
      let freshMap = {}
      try {
        const fRes = await sb.rpc('get_project_freshness')
        if (fRes.data) {
          for (const row of fRes.data) freshMap[row.project_id] = row.last_updated
        }
      } catch (_) {
        // Fallback: traer solo id+project_id+updated_at con limit alto
  ...
  loadProject: async (projectId) => {
    ...
    if (!project) return
    try {
      // Traer tareas del proyecto (con limit alto por seguridad)
      const tRes = await sb.from('tasks').select('*').eq('project_id', projectId).order('order_index').limit(5000)
      if (tRes.error) throw tRes.error
      const tasks = tRes.data || []
      const ids = new Set(tasks.map(t => t.id))

      // Dependencias: solo las que involucran tareas de este proyecto
  ...
  outdentTask: async (taskId) => {
    ...
    // Hermanas que vienen DESPUÉS de X (mismo padre, order_index mayor) → pasan a ser hijas de X
    const siblings = (childrenMap[task.parent_task_id || 'ROOT'] || [])
      .sort((a, b) => (a.order_index || 0) - (b.order_index || 0))
    const idx = siblings.findIndex(s => s.id === taskId)
    const followers = siblings.slice(idx + 1)

    // 1. X sube un nivel (padre → abuelo)
    await sb.from('tasks').update({ parent_task_id: grandParentId }).eq('id', taskId)

    // 2. Las hermanas siguientes pasan a ser hijas de X (adopción estilo Project)
    for (const f of followers) {
      await sb.from('tasks').update({ parent_task_id: taskId }).eq('id', f.id)
    }
